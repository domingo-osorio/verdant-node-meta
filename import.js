...
const meta_data = await google_sheets.spreadsheets.get({ spreadsheetId });
const sheet_names = meta_data.data.sheets.map(x => x.properties.title);

let entries = await Promise.all(
    sheet_names.map(async x => {
        const sheet_data = await google_sheets.spreadsheets.values.get({ spreadsheetId, range: x + "!A:ZZZ" });
